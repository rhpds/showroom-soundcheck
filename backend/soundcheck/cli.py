...
No dependency on Postgres or Babylon.
...
from .services.check_service import TargetCheckResult, check_targets
...
def _try_import_rich() -> tuple[Any | None, Any | None, Any | None]:
    try:
...
async def _run_checks(
    ...
    return await _run_plain(urls, check_type, verbose, verify_ssl, concurrency)
...
async def _run_with_rich(
    ...
    async def on_progress(url: str, status: str, result: TargetCheckResult | None) -> None:
        if status == "running":
            status_map[url] = "[blue]running...[/]"
