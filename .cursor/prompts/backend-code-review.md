...
See [AGENTS.md](../../AGENTS.md) for the tech stack, architectural rules (SAQ two-queue design, streaming-first/SSE, the workshops-dashboard-is-REST-only exception), and conventions -- this review assumes that context. Full file tree: [README.md](../../README.md#architecture).

**Full scope**: `backend/soundcheck/`, including `routes/workshops.py` and `services/workshop_service.py` (~700 lines -- the workshop dashboard's K8s fetching, status derivation, filtering, and its own in-memory cache with a background refresh loop started/stopped from `main.py`'s `lifespan()`) and `schemas_workshops.py`. These are real review targets for every dimension below (Security, Database, Error Handling, Code Quality, API Design), not just a footnote.
...
- **SQL injection**: While SQLModel/SQLAlchemy parameterize queries, check for any raw SQL or string interpolation in queries. `routes/workshops.py` has the one raw-SQL spot in the codebase -- a module-level `text(...)` query (`_CHECK_STATUS_QUERY`, joined against `json_array_elements_text`) used by `POST /workshops/check-status`. Verify `workshop_ids` is passed as a bound parameter (`:workshop_ids`) and never string-formatted into the query.

- **Secret handling**: Check `config.py` for hardcoded defaults (e.g., `soundcheck_dev` password). Check `babylon_client.py` for token/certificate handling and temp file cleanup for client certs.
...
- **Backpressure and connection limits**: SSE endpoints hold a long-lived connection + Redis subscription per connected client. A concurrent-connection cap already exists (`MAX_SSE_CONNECTIONS`, default 200, enforced via the `sse_capacity_guard` dependency in `routes/_sse.py`, which returns 503 once exhausted). Evaluate whether the default is appropriate, whether it's per-process (multi-worker Uvicorn deployments would multiply the effective limit), and what happens if a client connects but never reads (slow consumer).
...
- **Default credentials**: `config.py` defaults to `soundcheck_dev` password when `POSTGRES_PASSWORD` and `DATABASE_URL` are not set. This already fails closed (`warn_default_credentials()` raises `RuntimeError`) for any `ENVIRONMENT` other than `development`; it only warns-and-continues when `ENVIRONMENT=development`. Verify deployment manifests set `ENVIRONMENT` correctly (staging/production should never default to `development`), and that no code path constructs `Settings` in a way that skips calling `warn_default_credentials()` at startup.
...
- **Uvicorn configuration**: `entrypoint.sh` runs a single Uvicorn worker with no `--workers` flag (a previous attempt to add `UVICORN_WORKERS` was reverted). For production, multiple workers or a Gunicorn wrapper would be appropriate -- but note that SSE connections are per-process, which complicates multi-worker deployments. The workshop dashboard's in-memory cache (`workshop_service.py`'s `_ws_cache`/`_mws_cache` + `start_background_refresh()`/`stop_background_refresh()`, wired into `main.py`'s `lifespan()`) is also per-process: multiple Uvicorn workers would each run their own 55s-interval refresh loop against every configured Babylon cluster, multiplying K8s API load without multiplying cache freshness.
...
- **Minimal test suite**: Only `backend/tests/test_config.py` exists (covers `config.py` settings parsing) -- no `conftest.py`, no fixtures, and no route/service/task tests yet. Assess what testing strategy would be most impactful next. SAQ tasks should be unit-testable by mocking the `ctx` dict (with `session_factory` and `redis`). Integration tests for routes should verify jobs are enqueued correctly. See `.cursor/prompts/backend-testing.md` for the detailed plan.
