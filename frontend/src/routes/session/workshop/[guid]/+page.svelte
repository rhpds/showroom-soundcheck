<script lang="ts">
	...
	import { getOrCreateWorkshopSession } from '$lib/api';
	...
	onMount(() => {
		...
		const guid = page.params.guid?.trim() || '';
		if (!guid) {
			goto('/');
			return;
		}
		(async () => {
			try {
				...
				error = e instanceof Error ? e.message : 'Failed to open workshop session';
			}
		})();
...
{:else}
	<div class="pf-v6-u-text-align-center pf-v6-u-mt-2xl">
		<Spinner label="Opening workshop session" size="lg" text="Opening workshop session..." />
	</div>
{/if}
